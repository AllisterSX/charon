...
import { createDryRunPosition, createProbePosition, createLivePosition, canOpenMorePositions, openPositionCount, tradingMode } from '../db/positions.js';
import { fetchNetworkCongestion } from '../filters/networkCongestion.js';
...
export async function processCandidateFromSignals(signals) {
  ...
  const candidate = await buildCandidate({ ...signals, networkCongestion: await fetchNetworkCongestion().catch(() => null), ageMs: signals.ageMs });
  ...
  // Use strategy-level llm_min_confidence if set, otherwise fall back to global setting.
  // This allows per-strategy tuning (e.g. degen_micro uses 55, obicle_confirmed uses 65).
  // Note: strat is already declared above (line ~43), reuse it here.
  const minConfidence = (strat.llm_min_confidence != null && Number.isFinite(Number(strat.llm_min_confidence)))
...
export async function handleApprovedBuy(selectedRow, decision, batchId, rows = [], triggerCandidateId = null) {
  ...
  if (mode === 'dry_run') {
    ...
    const positionId = useProbe
      ? createProbePosition(freshSelectedRow.id, freshSelectedRow.candidate, decision, `llm_batch_${batchId}`)
      : await createDryRunPosition(freshSelectedRow.id, freshSelectedRow.candidate, decision, `llm_batch_${batchId}`);
    logDecisionEvent({
      batchId,
      triggerCandidateId,
      selectedRow: freshSelectedRow,
