...
async function triggerCandidate({ mint, fee, signature, graduatedCoin, trendingToken, route, ageMs }) {
  if (!candidateHandler) return;
  await candidateHandler({ mint, fee, signature, graduatedCoin, trendingToken, route, ageMs });
}
...
export async function fetchServerSignals() {
  try {
    ...
    for (const signal of signals) {
      ...
      // Strategy gate: token age (lower bound + upper bound)
      const tokenAge = signal.ageMs || 0;
      if (strat.token_age_min_ms > 0 && tokenAge > 0 && tokenAge < strat.token_age_min_ms) { processed++; continue; }
      if (strat.token_age_max_ms > 0 && tokenAge > 0 && tokenAge > strat.token_age_max_ms) { processed++; continue; }
      ...
      if (strat.entry_mode === 'wait_for_dip' && strat.max_ath_distance_pct < 0) {
        // Pre-filter: skip dip alert for obviously bad tokens (saves price monitor noise).
        // Full filter runs at trigger time via buildCandidate, but these are hard gates.
        const signalMcap = Number(signal.marketCapUsd ?? 0);
        const signalHolders = Number(signal.holders ?? 0);
        const dipMinMcap = strat.min_mcap_usd ?? 0;
        const dipMinHolders = strat.min_holders ?? 0;
        if (dipMinMcap > 0 && signalMcap > 0 && signalMcap < dipMinMcap) { processed++; continue; }
        if (dipMinHolders > 0 && signalHolders > 0 && signalHolders < dipMinHolders) { processed++; continue; }
        ...
        if (athDist != null && athDist <= strat.max_ath_distance_pct) {
          ...
          await triggerCandidate({ mint, fee, signature, graduatedCoin, trendingToken, route, ageMs: signal.ageMs });
          triggered++;
        } else {
