...
export function strategyMenuText() {
  ...
  if (isApex) {
    lines.push(
      ...
      `  Age: ${sfMs(strat.token_age_min_ms)} – ${sfMs(strat.token_age_max_ms)}`,
      `  Holders ≥ ${sf(strat.min_holders, '?')}  Top10 ≤ ${sfPct(strat.max_top10_holder_percent)}`,
      `  Fee/mcap ratio ≥ ${sf(strat.fee_to_mcap_min_ratio, '?')} (1:10K)${Number(strat.min_fee_claim_sol) > 0 ? `  min ${sf(strat.min_fee_claim_sol)} SOL` : ''}`,
      ``,
      `<b>Watchlist</b>`,
