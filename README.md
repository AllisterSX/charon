...
> **Phase 0 fork dari [yunus-0x/charon](https://github.com/yunus-0x/charon).** Versi ini di-customize untuk preferensi trading personal: 3 strategi tier-1 (`obicle_confirmed`, `graduation_pump`, `migration_play`), TA-based entry, defensive layers berdasarkan riset on-chain (Pine Analytics, MemeTrans paper, Mobula). Lihat `CHANGELOG-v2.md` untuk progres per phase.

Charon-v2 adalah Telegram trench agent untuk screening Solana memecoin dengan overlap signals (server agregasi 4-source: axiom, pump_graduated, jupiter_trending, fee_claim) + (planned) GMGN local poller, defensive filter layers, strategy gates, LLM pick, dan dry-run/confirm/live execution lewat Jupiter Ultra.
